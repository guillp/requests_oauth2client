...
- pass `dpop=True` when doing your token request with any of the `OAuth2Client` methods that send a token request,
...
The private key used for signing DPoP proofs is auto-generated by `OAuth2Client` whenever a new token is obtained.
By default, generated keys are of type Elliptic Curve and use the signature alg `ES256` (Elliptic-Curve with a SHA256 hash).
Should you wish to use a key that you generate yourself, you may instead use the parameter `dpop_key` to pass your own
generated key on any method that will trigger a token request. It takes a `DPoPKey` instance, which you can generate
...
Instead of generating your own keys everytime, you may also control how `DPoPKey`s are automatically generated.
...
You may also pass a custom `dpop_key_generator`, which is a callable that accepts a signature alg as parameter, and
generates `DPoPKey` instances. You may use `DPoPKey.generate` as a helper method for that, or implement your own
generator:
