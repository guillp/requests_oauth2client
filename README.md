...
- pass `dpop=True` when doing your token request,
...
A `DPoPToken` is actually a `BearerToken` subclass. If you use it as a `requests` auth handler, it will take care of
adding a DPoP proof to the request, in addition to the access token.

The private key that is used for signing DPoP proofs is auto-generated by `OAuth2Client` whenever a new token is obtained.
Generated keys use are of type Elliptic Curve and use the signature alg `ES256` (Elliptic-Curve with a SHA256 hash) by default.
You may instead force it to use a key and alg of your choice by using the parameter `dpop_key` on any method that will trigger a token
request. It takes a `DPoPKey` instance, which you can generate using `DPoPKey.generate()`, or by initializing an instance with
a key that you previously generated:
...
# but you need to specify the signature `alg`
...
### Hooking into automatic key generation

You may also control how `DPoPKey`s are automatically generated, which can be useful if you need to use a different proof
signing alg than the default `ES256`, or a custom `DPoPToken` class or other customizations to the DPoP proofing.
To do so, provide a custom `dpop_key_generator` to your `OAuth2Client` instance:
...
oauth2client = OAuth2Client.from_discovery_endpoint(
    ...
    client_id="client_id", client_secret="client_secret",
    dpop_bound_access_tokens=True,
    dpop_key_generator=lambda: DPoPKey.generate(
        alg="RS512",  # choose your alg, the appropriate key type will be automatically generated
        # those other parameters are for feature testing the AS, or for workarounding AS bugs:
        jwt_typ="jwt+custom", # you can customise the `typ` that is included in DPoP proof headers
        jti_generator=lambda: secrets.token_urlsafe(24), # or generate unique jti differently than the default UUIDs
        iat_generator=lambda: 12532424, # override `iat` generation in DPoP proofs
