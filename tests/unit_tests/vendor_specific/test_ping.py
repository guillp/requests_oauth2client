...
def test_ping_client() -> None:
    ...
    assert (
        ping_client.extra_metadata["registration_endpoint"]
        == "https://mydomain.tld/as/clients.oauth2"
    )
    assert (
    ...
    assert (
        ping_client.extra_metadata["ping_end_session_endpoint"]
        == "https://mydomain.tld/idp/startSLO.ping"
    )
    assert (
